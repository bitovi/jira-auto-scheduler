...
export default function JiraOIDCHelpers({ JIRA_CLIENT_ID, JIRA_SCOPE, JIRA_CALLBACK_URL, JIRA_API_URL } = window.env) {
  ...
  const jiraHelpers = {
    ...
    fetchAuthorizationCode: () => {
      const url = `https://auth.atlassian.com/authorize?audience=api.atlassian.com&client_id=${JIRA_CLIENT_ID}&scope=${escape(
        JIRA_SCOPE
      )}&redirect_uri=${JIRA_CALLBACK_URL}&response_type=code&prompt=consent&state=${encodeURIComponent(
    ...
    fetchJiraIssuesWithJQL: function (params) {
      ...
      return fetchJSON(`${JIRA_API_URL}/${scopeIdForJira}/rest/api/3/search?` + new URLSearchParams(params), {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
    ...
    fetchJiraChangelog(issueIdOrKey, params) {
      ...
      return fetchJSON(
        `${JIRA_API_URL}/${scopeIdForJira}/rest/api/3/issue/${issueIdOrKey}/changelog?` + new URLSearchParams(params),
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
    ...
    fetchJiraFields() {
      ...
      return fetchJSON(`${JIRA_API_URL}/${scopeIdForJira}/rest/api/3/field`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
    ...
    getServerInfo() {
      ...
      return fetchJSON(`${JIRA_API_URL}/${scopeIdForJira}/rest/api/3/serverInfo`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
